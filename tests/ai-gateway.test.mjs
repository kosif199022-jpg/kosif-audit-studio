...
test('realtime session uses a fixed safe voice contract and never returns the provider key',async()=>{
 ...
 assert.equal(response.status,200);assert.equal(sent.url,'https://api.openai.com/v1/realtime/calls');assert.equal(sent.options.body instanceof FormData,true);assert.equal(typeof sent.options.body.get('sdp'),'string');assert.equal(typeof sent.options.body.get('session'),'string');assert.equal(sent.options.body.get('session').includes('gpt-realtime-2.1'),true);assert.equal(sent.options.headers.accept,'application/sdp');assert.match(sent.options.headers['OpenAI-Safety-Identifier'],/^[a-f0-9]{32}$/);assert.ok(!JSON.stringify(session).includes(testKey));
});

test('server realtime secret works without exposing it or requiring a BYOK session',async()=>{
 const {env}=setup();env.OPENAI_API_KEY='server-key-never-returned-12345';env.OPENAI_MODEL='gpt-realtime-2.1';
 const publicConfig=await (await handleAi(req('config'),env)).json();assert.equal(publicConfig.providers.find(({id})=>id==='openai').configured,true);assert.equal(JSON.stringify(publicConfig).includes(env.OPENAI_API_KEY),false);
 let sent;const response=await handleAi(new Request(origin+'/api/ai/realtime',{method:'POST',headers:{origin,'content-type':'application/json'},body:JSON.stringify({sdp:'v=0\\r\\nm=audio 9 RTP/AVP 0',consent:true})}),env,async(url,options)=>{sent=options;return new Response('v=0\\r\\nm=audio 9 RTP/AVP 0');});
 assert.equal(response.status,200);assert.equal(sent.headers.authorization,'Bearer '+env.OPENAI_API_KEY);assert.equal(JSON.stringify(await response.json()).includes(env.OPENAI_API_KEY),false);
});
