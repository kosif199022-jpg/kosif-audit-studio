...
test('Spatial preferences tolerate unavailable or corrupt storage and preserve disabling effects',()=>{
 assert.deepEqual(readSpatial(null),{enabled:true,motion:true,cinematic:true,url:''});
 assert.deepEqual(readSpatial({getItem:()=>'{broken'}),{enabled:true,motion:true,cinematic:true,url:''});
 assert.deepEqual(readSpatial({getItem:()=>JSON.stringify({enabled:false,motion:false,cinematic:false,url:'https://evil.test'})}),{enabled:false,motion:false,cinematic:false,url:''});
});
