...
export async function extractAuditDocument(file, { maxPages = 48, startPage = 1, store = storeEvidenceBytes } = {}) {
  ...
  let extracted;
  if (/\.pdf$/i.test(file.name)) extracted = await extractPdf(file, maxPages, startPage);
    ...
    const parts = [...doc.getElementsByTagName('w:p')].map((p, i) => ({ page: i + 1, locator: 'DOCX فقرة ' + (i + 1), text: [...p.getElementsByTagName('w:t')].map(t => t.textContent || '').join(' ') }));
    ...
    const parts = workbook.SheetNames.slice(0, 12).flatMap(name => XLSX.utils.sheet_to_json(workbook.Sheets[name], { header: 1, raw: false, defval: '' }).slice(0, 1000).map((row, i) => ({ page: i + 1, locator: name + ' · صف ' + (i + 1), text: row.map(clean).join(' | ') })));
    extracted = { parts, pages: 0, pagesRead: 0, method: 'xlsx-rows', limit: 'أول 1000 صف من أول 12 ورقة' };
  } else if (/\.(csv|tsv|txt|json)$/i.test(file.name)) {
    const text = await file.text();
    if (/\.json$/i.test(file.name)) JSON.parse(text);
    extracted = { parts: text.split(/\r?\n/).slice(0, 20000).map((line, i) => ({ page: i + 1, locator: 'سطر ' + (i + 1), text: line })), pages: 0, pagesRead: 0, method: 'text-lines', limit: 'أول 20000 سطر' };
  } else throw new Error('استخدم PDF نصيًا أو DOCX أو XLSX أو CSV أو TXT أو JSON.');
  ...
  return { id, documentId, name: file.name, size: file.size, method: extracted.method, pages: extracted.pages, pagesRead: extracted.pagesRead, startPage: extracted.startPage, endPage: extracted.endPage, limit: extracted.limit, snippetCount: snippets.length, snippets, selected: [], createdAt: new Date().toISOString() };
}
...
export function selectedDocumentExcerpts(documents = []) {
  return documents.flatMap(doc => (doc.selected || []).map(index => doc.snippets?.[index]).filter(Boolean).map(item => ({ documentId: item.documentId, page: item.page, locator: item.locator, text: item.text }))).slice(0, 8);
}
