...
export function answerAgent(query, c = {}) {
 ...
 if (/دليل|ادله|فجوات|تغطيه/.test(q)) {
  ...
  return result('evidence', `${evidence.length} سجل دليل، منها ${adequate.length} ذو مراجعة وبصمة موثقتين في السجل. ${gaps} خطرًا بلا دليل بهذه الصفات. يجب إعادة فحص البايتات المحتفظ بها عند الاعتماد؛ كفاية الدليل وملاءمته يقيّمهما المراجع.`, ['socpa-audit'], [{ label: 'فحص الأدلة', view: 'evidence' }]);
 }
