...
export function readSpatial(storage) {
 try { const s=JSON.parse(storage.getItem(SPATIAL_KEY)||'{}');return {enabled:s.enabled!==false,motion:s.motion!==false,cinematic:s.cinematic!==false,url:splineUrl(s.url)}; } catch {return {enabled:true,motion:true,cinematic:true,url:''};}
}
