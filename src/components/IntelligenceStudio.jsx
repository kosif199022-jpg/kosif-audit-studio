...
import { BrainCircuit, Mic, Square, Send, Download, Plus, Globe2, LoaderCircle } from 'lucide-react';
...
const tabs = [['agent','الإيجنت'],['plan','خطة العمل'],['council','تحديات التخصصات'],['research','بحث مهني'],['references','مرصد المراجع'],['memos','المذكرات']];
...
export function IntelligenceStudio({ accounts, engagement, setEngagement, metrics, reportState, onView, onToast }) {
 ...
 const [researchQuestion,setResearchQuestion] = useState('ما متطلبات توثيق تقديرات القيمة العادلة في القوائم المالية وفق IFRS والمعايير المهنية ذات الصلة؟');
 ...
 async function runResearch(){const questionText=researchQuestion.trim().slice(0,1500);if(questionText.length<3)return onToast?.('اكتب سؤالًا مهنيًا للبحث.');if(!researchConsent)return onToast?.('وافق على إرسال سؤال البحث إلى OpenAI.');setResearchBusy(true);try{setResearchResult(await aiRequest('research',{question:questionText,consent:true}));}catch(error){setResearchResult({ok:false,text:error.message,citations:[]});}finally{setResearchBusy(false);}}
 return <section className="ki-studio" aria-labelledby="ki-title">
  ...
  {tab==='research' && <div><div className="panel ki-card"><span className="eyebrow"><Globe2 size={14}/> مصادر مهنية مباشرة</span><h3>ابحث في المعايير والمصادر الرسمية</h3><p>يبحث المسار في نطاقات مهنية مسموحة فقط، ويعرض الروابط التي أعادها المصدر. لا يُدخل المستندات أو مفاتيحك في البحث.</p><label>سؤال البحث<textarea value={researchQuestion} maxLength={1500} onChange={e=>setResearchQuestion(e.target.value)}/></label><label className="ki-check"><input type="checkbox" checked={researchConsent} onChange={e=>setResearchConsent(e.target.checked)}/> أوافق على إرسال السؤال إلى OpenAI وقد تُطبق رسوم المزوّد.</label><div className="button-row"><button className="button button-dark" type="button" disabled={researchBusy||!researchConsent} onClick={runResearch}>{researchBusy?<><LoaderCircle className="spin" size={16}/> جارٍ البحث…</>:<><Globe2 size={16}/> تشغيل البحث</>}</button></div></div>{researchResult&&<article className="ki-card"><h3>نتيجة البحث</h3><p className="ki-preserve">{researchResult.text||'لم تُنتج النتيجة نصًا.'}</p><small>{researchResult.authority==='advisory-only'?'إجابة استشارية؛ تحقق من النص الساري واعتماد بلدك.':''} · {researchResult.generatedAt||''}</small><SourceLinks items={(researchResult.citations||[]).map((item,index)=>({id:item.url||String(index),code:item.title||'مصدر رسمي',url:item.url}))}/></article>}</div>}
