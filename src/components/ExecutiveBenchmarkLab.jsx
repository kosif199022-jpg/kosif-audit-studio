...
import { BENCHMARK_DIMENSIONS, REFERENCE_PACKET, buildAiCompanyContext, buildCompanyReportText, buildLocalAiReview, getBenchmarkCompanies, getBenchmarkReport, getBenchmarkReports, runCompanyAnalysis } from "../report-benchmark.js";
...
export function ExecutiveBenchmarkLab({ initialReportId = "apple-2024", engagement, setEngagement, metrics, reportState, onView, onToast, onOpenStandard }) {
  const initialReport = getBenchmarkReport(initialReportId);
  const [selected, setSelected] = useState(() => initialReport);
  const [overrides, setOverrides] = useState({});
  // Seed the result from the requested report as well as the selector.  This
  // keeps a company opened from the overview truthful on its first paint
  // instead of briefly showing Apple's numbers for every company.
  const [analysis, setAnalysis] = useState(() => runCompanyAnalysis(initialReport.id));
  ...
  useEffect(() => {
    const next = getBenchmarkReport(initialReportId);
    if (next.id !== selected.id) choose(next);
    // The parent changes initialReportId only when the user selects a company
    // from the overview showcase; manual selector changes remain local.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [initialReportId]);

  function connectAi() {
    setEngagement?.((current) => ({ ...current, aiCompanyContext: buildAiCompanyContext(analysis) }));
    onView?.("ai-connections");
  }
  ...
  return <div className="view-stack benchmark-lab">
    ...
    <section className="benchmark-two-col"><article className="panel benchmark-card"><div className="section-heading"><div><span className="eyebrow">3 · المقارنة الزمنية</span><h2>حركة الأرقام</h2></div><RefreshCcw size={23} aria-hidden="true" /></div><div className="benchmark-change-list">{analysis.changeRows.slice(0, 10).map(row => <div key={row.key}><span>{row.label}</span><b>{format(row.current)}</b><small>{row.change >= 0 ? "▲" : "▼"} {Math.abs(row.changePct)}% مقابل السابق</small></div>)}</div></article><article className="panel benchmark-card"><div className="section-heading"><div><span className="eyebrow">4 · ربط المعيار</span><h2>لماذا ظهرت النتيجة؟</h2></div><BrainCircuit size={23} aria-hidden="true" /></div><div className="benchmark-standard-list">{analysis.standards.map(item => <div key={item.standardId}><button type="button" className="text-button" onClick={() => onOpenStandard?.(item.standardId)}>{item.standardId} ↗</button><p>{item.rationale}</p><small>{item.title}</small></div>)}</div></article></section>
