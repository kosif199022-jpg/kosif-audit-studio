...
for(const href of ['./freshness-specialists.css','./issue-workspace.css','./council-insights.css','./statement-comparison.css','./report-intake.css','./disclosure-workspace.css'])if(!document.querySelector(`link[href="${href}"]`)){const link=document.createElement('link');link.rel='stylesheet';link.href=href;document.head.append(link)}
function renderAll(options={}){reconcileReportIntake();derive();renderNow();renderAnalysisMetrics();renderDocuments();renderRecipe();renderReportIntake();renderCouncil();renderRequests();renderIssues();renderCoverage();renderCandidates();renderAdjustments();renderStatements();renderDisclosures();renderTrace();if(store.engagement.reports.length)renderReport();renderCompletion();if(options.persist!==false)persistence?.schedule()}
initDocuments({renderAll});initCouncil({renderAll});initIssues({renderAll});initCoverage({renderAll});initStatements();initReporting({renderAll});initReportIntake({renderAll});initDisclosures({renderAll});initCompletion({renderAll});persistence=initPersistence({renderAll});
