...
for(const href of ['./freshness-specialists.css','./issue-workspace.css'])if(!document.querySelector(`link[href="${href}"]`)){const link=document.createElement('link');link.rel='stylesheet';link.href=href;document.head.append(link)}
function renderAll(options={}){derive();renderNow();renderAnalysisMetrics();renderDocuments();renderRecipe();renderCouncil();renderRequests();renderIssues();renderCandidates();renderAdjustments();renderTrace();if(store.engagement.reports.length)renderReport();renderCompletion();if(options.persist!==false)persistence?.schedule()}
initDocuments({renderAll});initCouncil({renderAll});initIssues({renderAll});initReporting({renderAll});initCompletion({renderAll});persistence=initPersistence({renderAll});
