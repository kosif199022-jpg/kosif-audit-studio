...
// The container keeps the WhatsApp auth state in memory only; this Durable Object
// persists it in its own SQLite storage and restores it whenever the container restarts.
...
export class WaBridge extends Container<Env> {
  defaultPort = 8080;
  sleepAfter = '10m'; // no keep-alive: the container wakes on demand and sleeps when idle
...
function openapi(origin: string) {
  return {
    openapi: '3.1.0',
    info: { title: 'WhatsApp Bridge', version: '1.0.0', description: 'Send WhatsApp messages from the linked personal number.' },
    servers: [{ url: origin }],
    ...
    paths: {
      '/send': {
        post: {
            ...
            content: {
              'application/json': {
                schema: {
                  ...
                  required: ['to', 'message'],
                  properties: {
                    to: { type: 'string', description: 'Phone number in international format, digits only, e.g. 201012345678' },
          ...
          responses: { '200': { description: 'Sent' }, '404': { description: 'Number is not on WhatsApp' } },
        },
      },
      '/status': {
        get: { operationId: 'getWhatsAppStatus', summary: 'Whether the WhatsApp number is linked', responses: { '200': { description: 'Status' } } },
      },
    },
  };
}
...
p{max-width:420px;text-align:center;line-height:1.7}</style></head>
...
if(d.linked){document.getElementById('qr').textContent='✅ تم الربط';document.getElementById('msg').textContent='الرقم مربوط. تقدر تقفل الصفحة دي.';return;}
